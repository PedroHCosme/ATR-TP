 ...
 * @brief Classe responsável pela sincronização e notificação de eventos críticos do sistema.
 * 
 * Esta classe atua como um monitor thread-safe para gerenciar o estado de falhas
 * do sistema. Permite que tarefas sinalizem a ocorrência de falhas e que outras
 * tarefas verifiquem ou aguardem por esses eventos.
 */
class EventosSistema {
private:
    std::mutex mtx; ///< Mutex para proteção de acesso concorrente.
    std::condition_variable cv_falha; ///< Variável de condição para notificação de falhas.
    bool falha_critica_detectada = false; ///< Flag indicando se uma falha crítica está ativa.
    int codigo_falha = 0; ///< Código identificador da falha (Ex: 1=Temp, 2=Eletrica, 3=Hidraulica).

public:
    /**
     * @brief Sinaliza a ocorrência de uma falha no sistema.
     * 
     * Este método deve ser chamado pelas tarefas de monitoramento quando uma anomalia
     * é detectada. Ele atualiza o estado interno e notifica as threads interessadas.
     * 
