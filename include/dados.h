...
struct DadosSensores {
  ...
  int i_lidar_distancia; // Distância medida pelo LiDAR em metros (0 a 100)

  int o_aceleracao; // Determina a aceleração do veículo em
                    //    percentual (-100 a 100%)

  int o_direcao; // Determina a direção do veículo em graus (-180 a 180°). Ao
                 // acelerar, o veículo se moverá nessa direção.
};
 ...
 * @brief Armazena os comandos de controle gerados pela navegação para os
 * atuadores.
 */
struct ComandosAtuador {
...
/**
 * @struct ObjetivoNavegacao
 * @brief Define o objetivo atual de navegação para o controlador.
 */
struct ObjetivoNavegacao {
  bool ativo;            // Se true, há um destino definido.
  float x_alvo;          // Coordenada X do alvo.
  float y_alvo;          // Coordenada Y do alvo.
  float velocidade_alvo; // Velocidade desejada para este segmento.
};
...
struct CaminhaoFisico {
  ...
  float i_lidar_distancia;        ///< Distância simulada para obstáculo (m).
};
