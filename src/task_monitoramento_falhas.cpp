...
void task_monitoramento_falhas(GerenciadorDados &gerenciadorDados,
                               EventosSistema &eventos, ISensorDriver &driver) {
  ...
  loop_monitoramento = [&]() {
    ...
    // Leitura do Buffer apenas para dados não-críticos ou redundância
    // (opcional) DadosSensores sensores = gerenciadorDados.lerUltimoEstado();
    ...
    // std::cout << "[DEBUG-MONITOR] Temperatura Real (Driver): " <<
    ...
    if (estadoReal.i_temperatura > 120) {
      ...
      // std::cout << "[MONITOR] ALERTA: Motor aquecendo ("
      //           << estadoReal.i_temperatura << " C)..." << std::endl;
    }

    // 3. Verificação de Subsistemas (Elétrica/Hidráulica)
    if (estadoReal.i_falha_eletrica) {
      // std::cout << "[MONITOR] CRITICO: Falha Eletrica detectada!" <<
      // std::endl;
      nova_falha_detectada = true;
    ...
    if (estadoReal.i_falha_hidraulica) {
      // std::cout << "[MONITOR] CRITICO: Falha Hidraulica detectada!"
      //           << std::endl;
      nova_falha_detectada = true;
    ...
    if (nova_falha_detectada) {
      if (!eventos.verificar_estado_falha()) { // Só sinaliza se ainda não
                                               // estiver em falha
        eventos.sinalizar_falha(codigo_falha);
        // std::cout << "[MONITOR] Falha sinalizada via EventosSistema (Codigo "
        //           << codigo_falha << ")." << std::endl;
      }
    }

    // (Opcional) Lógica de recuperação automática poderia vir aqui,
    // mas em sistemas críticos, geralmente o reset é manual (Comando de
    // Rearme).
