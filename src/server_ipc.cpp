...
std::string build_json(const CaminhaoFisico& caminhao, const EstadoVeiculo& estado, const std::vector<std::vector<char>>& mapa, bool send_map) {
    std::stringstream ss;
    ss << "{";
    ...
    // Mapa (enviar apenas uma vez ou se solicitado seria melhor, mas aqui simplificamos)
    if (send_map) {
        ss << ", \"map\": [";
        for (size_t y = 0; y < mapa.size(); ++y) {
            ss << "\"";
            for (char c : mapa[y]) ss << c;
            ss << "\"";
            if (y < mapa.size() - 1) ss << ",";
        }
        ss << "]";
...
ServerIPC::ServerIPC(GerenciadorDados& d, SimulacaoMina& s, EventosSistema& e, const std::vector<std::vector<char>>& m)
    : server_fd(-1), client_fd(-1), running(false), dados(d), simulacao(s), eventos(e), mapa(m) {}
...
void ServerIPC::loop() {
    ...
    std::cout << "[IPC] Aguardando conexao Python na porta 65432..." << std::endl;
...
void ServerIPC::handle_client() {
    ...
    while (running) {
        ...
        if (send(client_fd, &len, 4, MSG_NOSIGNAL) < 0) break;
        if (send(client_fd, json.c_str(), json.size(), MSG_NOSIGNAL) < 0) break;

        // 2. Recebe Comandos (Non-blocking check seria ideal, mas aqui vamos fazer polling simples ou thread separada de leitura)
        // Para simplificar, vamos assumir que o Python manda comandos esporadicamente.
        // Mas recv bloqueia. Precisamos de select ou non-blocking.
...
void ServerIPC::process_command(const std::string& json) {
    ...
    if (json.find("\"mode\": \"auto\"") != std::string::npos) {
        ...
        if (json.find("\"kind\": \"electric\"") != std::string::npos) eventos.sinalizar_falha(2);
        if (json.find("\"kind\": \"hydraulic\"") != std::string::npos) eventos.sinalizar_falha(3);
        ...
        if (json.find("\"accel\": 1") != std::string::npos) cmd.c_acelerar = true;
        if (json.find("\"steer\": 1") != std::string::npos) cmd.c_direita = true;
        if (json.find("\"steer\": -1") != std::string::npos) cmd.c_esquerda = true;
    }
