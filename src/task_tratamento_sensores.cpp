 ...
 */
float calcular_media_movel_exponencial(float valor_atual,
 ...
 *
 * Esta função simula a leitura de sensores de um caminhão em movimento dentro
 * do mapa gerado. Agora ela atua como uma "leitora burra", consultando a
 * simulação física.
 *
...
void task_tratamento_sensores(GerenciadorDados &gerenciadorDados,
                              ISensorDriver &driver, int id_caminhao) {
  ...
  while (true) {
    ...
    if (primeira_leitura) {
      // Na primeira leitura, inicializamos a média com o valor atual para
      // evitar convergência lenta
      ...
      ema_pos_x = calcular_media_movel_exponencial(
          static_cast<float>(raw_pos_x), ema_pos_x);
      ema_pos_y = calcular_media_movel_exponencial(
          static_cast<float>(raw_pos_y), ema_pos_y);
      ema_ang_x = calcular_media_movel_exponencial(
          static_cast<float>(raw_ang_x), ema_ang_x);
    }
    ...
    novosDados.i_velocidade = static_cast<int>(
        estadoReal.velocidade); // Copia do físico para o sensor
